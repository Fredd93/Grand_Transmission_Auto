...
class CarApiController
{
    ...
    public function deleteCar($id)
    {
        try {
            if (!is_numeric($id)) {
                ResponseHelper::sendError("Invalid car ID", 400);
                return;
            }
        
            $carId = (int)$id;
            $success = $this->carModel->deleteCarById($carId);
        
            if ($success) {
                ResponseHelper::sendJson(['success' => true, 'message' => 'Car deleted successfully']);
            } else {
                ResponseHelper::sendError('Failed to delete car', 500);
            }
        } catch (Exception $e) {
